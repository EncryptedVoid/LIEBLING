...
export default function EventsPage() {
  ...
  async function fetchData() {
    ...
    if (friendships && friendships.length > 0) {
      ...
      const profileMap = new Map(
        (fProfilesRes.data ?? []).map((p: any) => [p.id, p])
      );

      const fEvents: EventWithMeta[] = (fEventsRes.data ?? []).map(
        (e: any) => ({
          ...e,
          collection_name: e.collections?.name ?? null,
          collections: undefined,
          owner: profileMap.get(e.user_id) ?? undefined,
        })
      );
  ...
  const allPast = [...events, ...friendEvents]
    ...
    if (items.length === 0) {
      return (
        <div className="flex flex-col items-center py-16">
          <p className="text-muted-foreground text-sm">No events here.</p>
        </div>
      );
    }
    return (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-0.5 text-xs">
            Your occasions and your friends&apos; too.
          </p>
        </div>
      ...
      <Tabs defaultValue="mine" className="mt-6">
        <TabsList>
          <TabsTrigger value="mine">
            My Events ({myUpcoming.length})
          </TabsTrigger>
          <TabsTrigger value="friends">
            Friends&apos; Events ({friendUpcoming.length})
          </TabsTrigger>
          <TabsTrigger value="past">Past ({allPast.length})</TabsTrigger>
        </TabsList>
        <TabsContent value="mine" className="mt-4">
          <EventList items={myUpcoming} editable />
        </TabsContent>
        <TabsContent value="friends" className="mt-4">
          <EventList items={friendUpcoming} showOwner />
        </TabsContent>
